...
/*
  ...
  Maintenance History:
  ====================
  ver 3.6 : 5 March 2019
  - Small changes made into PreProcStatements and HandlePreProcStatements class
	to handle #include statements and exclude #def statements
  ver 3.5 : 11 Feb 2019
...
{  
  ...
  public:
    bool doTest(const Lexer::ITokenCollection* pTc) override
    {
	  ...
	  if (pPos < pTc->size())
	  {
		  size_t preprocFound, udefpreprocFound;
		  pTc->find("include", preprocFound);
		  pTc->find("<", udefpreprocFound);
		  if (preprocFound < pTc->size() && udefpreprocFound >= pTc->size()) {
			  doActions(pTc);
			  return IRule::Stop;
		  }
	  }
 ...
 ///////////////////////////////////////////////////////////////////
 ////// rule to detect comments

 // class Comment : public IRule
 // {
 // public:
	//  bool doTest(const Lexer::ITokenCollection* pTc) override
	//  {
 ...
 // class HandleComment : public IAction
 // {
	//  Repository* p_Repos;
 // public:
	//  HandleComment(Repository* pRepos)
	//  {
	//	  p_Repos = pRepos;
	//  }
	//  void doAction(const Lexer::ITokenCollection* pTc) override
	//  {
	//		  GrammarHelper::showParseDemo("handle comments", *pTc);
	...
	//		  pElem->name_ = "comment";
	//		  pElem->package_ = p_Repos->package();
	//		  if (p_Repos->scopeStack().top()->children_.size() != 0 && p_Repos->scopeStack().top()->statements_.size() == 0) {
	//			  temp = p_Repos->scopeStack().top()->children_[p_Repos->scopeStack().top()->children_.size() - 1];
	//			  pElem->startLineCount_ = temp->endLineCount_ + 1 + p_Repos->scopeStack().top()->statements_.size();
	//		  }
	//		  else
	//			  pElem->startLineCount_ = p_Repos->lineCount();
	//		  pElem->endLineCount_ = p_Repos->lineCount();

	//		 /* pElem->startLineCount_ = p_Repos->scopeStack().top()->endLineCount_ + 1 + p_Repos->scopeStack().top()->statements_.size();
	//		  pElem->endLineCount_ = p_Repos->lineCount();*/

	//		  pElem->path_ = p_Repos->currentPath();

	//		  p_Repos->scopeStack().top()->children_.push_back(pElem);
